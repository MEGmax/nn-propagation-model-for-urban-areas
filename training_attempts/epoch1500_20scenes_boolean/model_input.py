...
def load_scene_payload(scene_dir: Path, mask_dir: Path | None = None) -> dict:
    ...
    pathloss_db = np.clip(pathloss_db, a_min=None, a_max=110.0)
    ...
    # Load mask if mask_dir provided — mask value 1 = exclude pixel
    mask = None
    if mask_dir is not None:
        mask_path = mask_dir / f"{scene_dir.name}_mask.npy"
        if mask_path.exists():
            mask = np.load(mask_path).astype(bool)
            if mask.shape != pathloss_db.shape:
                # Resize mask to match pathloss shape if needed
                mask = resize(mask.astype(np.float32), pathloss_db.shape, order=0,
                              preserve_range=True, anti_aliasing=False).astype(bool)
            print(f"  Loaded mask for {scene_dir.name}: {mask.sum()} pixels excluded ({100*mask.mean():.1f}%)")
        else:
            print(f"  No mask found for {scene_dir.name} at {mask_path}, using all pixels")

    return {
        "scene_name": scene_dir.name,
        "elevation_m": elevation,
        "pathloss_db": pathloss_db,
        "mask": mask,  # None or bool array (H, W), True = exclude
...
def collect_stats(scene_payloads: list[dict]) -> object:
    ...
    total_pixels = 0          # for pathloss (masked)
    total_pixels_all = 0      # for elevation + electrical distance (unmasked)
    pathloss_sum = 0.0
    pathloss_sq_sum = 0.0
    elevation_sum = 0.0
    ...
    for payload in scene_payloads:
        ...
        # Build a boolean keep-mask for pathloss only: True = include in stats
        mask = payload.get("mask")  # True = exclude
        if mask is not None:
            keep = ~mask  # True = include
        else:
            keep = np.ones(pathloss_db.shape, dtype=bool)

        n_valid = int(keep.sum())
        if n_valid == 0:
            raise ValueError(f"Scene {payload['scene_name']} has no valid pixels after masking")

        # Pathloss stats: only over unmasked pixels
        total_pixels += n_valid
        total_pixels_all += pathloss_db.size
        pathloss_sum += float(pathloss_db[keep].sum())
        pathloss_sq_sum += float(np.square(pathloss_db[keep]).sum())

        # Elevation and electrical distance stats: all pixels (mask does not apply)
    ...
    std_db = variance**0.5
    elevation_mean_m = elevation_sum / total_pixels_all
    elevation_variance = max(elevation_sq_sum / total_pixels_all - elevation_mean_m**2, 1e-8)
    elevation_std_m = elevation_variance**0.5
    electrical_distance_mean = electrical_distance_sum / total_pixels_all
    electrical_distance_variance = max(
        electrical_distance_sq_sum / total_pixels_all - electrical_distance_mean**2,
        1e-8,
    )
...
def preprocess_scenes(
    ...
    scene_payloads = [load_scene_payload(scene_dir, mask_dir=mask_dir) for scene_dir in scene_dirs]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory containing boolean mask files (e.g. scene0_mask.npy). "
            "Pixels with value 1 are excluded from normalization stat computation."
        ),
    )
    parser.add_argument(
        "--stats-file-in",
        ...
        default=None,
        help=(
            "Path to an EXISTING normalization stats file (e.g. from the training split). "
            "If provided, stats are loaded from this file instead of being recomputed. "
            "Use this for val and test splits to ensure consistent normalization."
        ),
    )
